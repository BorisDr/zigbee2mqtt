...
export type HubToInstanceMessage = {type: "open"; id: number} | {type: "message"; id: number; data: string} | {type: "close"; id: number};

/** Messages sent from an instance to the combined frontend (supervisor). */
export type InstanceToHubMessage = {type: "send"; id: number; data: string} | {type: "broadcast"; data: string} | {type: "closeAll"; data: string};

let context: InstanceContext | undefined;

export function setInstanceContext(newContext: InstanceContext | undefined): void {
    context = newContext;
}

export function getInstanceContext(): InstanceContext | undefined {
    return context;
}
